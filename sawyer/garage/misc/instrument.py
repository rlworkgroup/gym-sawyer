...
from sawyer.garage.import config
from sawyer.garage.core import Serializable
from sawyer.garage.misc.console import mkdir_p
from sawyer.garage.misc.ext import AttrDict
from sawyer.garage.viskit.core import flatten
...
def launch_ec2(params_list,
    ...
    if config.FAST_CODE_SYNC:
        # sio.write("""
        #     aws s3 cp {code_full_path} /tmp/sawyer.garage.code.tar.gz --region {aws_region}  # noqa: E501
        # """.format(code_full_path=code_full_path, local_code_path=config.DOCKER_CODE_DIR,  # noqa: E501
        #            aws_region=config.AWS_REGION_NAME))
        sio.write("""
            aws s3 cp {code_full_path} /tmp/sawyer.garage.code.tar.gz
        """.format(
        ...
        """.format(
            ...
            aws_region=config.AWS_REGION_NAME))
        sio.write("""
            tar -zxvf /tmp/sawyer.garage.code.tar.gz -C {local_code_path}
        """.format(
            code_full_path=code_full_path,
            local_code_path=config.DOCKER_CODE_DIR,
            aws_region=config.AWS_REGION_NAME))
...
def to_lab_kube_pod(params,
    ...
    if config.FAST_CODE_SYNC:
        pre_commands.append(
            'aws s3 cp %s /tmp/sawyer.garage.code.tar.gz' % code_full_path)
        pre_commands.append('mkdir -p %s' % config.DOCKER_CODE_DIR)
        pre_commands.append(
            'tar -zxvf /tmp/sawyer.garage.code.tar.gz -C %s' % config.DOCKER_CODE_DIR)
    else:
