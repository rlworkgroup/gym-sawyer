...
class BlockWorld(World):
    ...
    def initialize(self):
        ...
        if self._simulated:
            ...
            self._tf_listener.waitForTransform("base_d", "block_0", rospy.Time(0), rospy.Duration(1000))
            tf_topic = "/tf"            
            try:
        ...
        if self._tf_listener.frameExists(robot_frame):
            for block in self._blocks:
                ...
                if self._tf_listener.frameExists(block_frame):
                    translation_wrt_robot, orientation_wrt_robot = self._tf_listener.lookupTransform(robot_frame, block_frame, rospy.Time(0))
                    ...
                    block.position.x = translation_wrt_robot.x
                    block.position.y = translation_wrt_robot.y
                    block.position.z = translation_wrt_robot.z
                    #block.position.z = 0.065
                    block.orientation.x = orientation_wrt_robot.x
                    block.orientation.y = orientation_wrt_robot.y
                    block.orientation.z = orientation_wrt_robot.z
                    block.orientation.w = orientation_wrt_robot.w
                    self._moveit_update_block(block)
                    print("UPDATING BLOCK POS: " + str(block.position))
                else:
                    print("block frame does not exists.")
        else:
            print("base frame does not exists.")
    ...
    def get_block_position(self, name):
        for block in self._blocks:
            if block.name == name:
                print("BLOCK POSITION IS: " + str(block.position))
